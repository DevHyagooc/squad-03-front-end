...
export function RepactuacoesContrato({ contratoId }: RepactuacoesContratoProps) {
  ...
  const [repactuacaoToDelete, setRepactuacaoToDelete] = useState<{ id: number; indice: string } | null>(null)

  // Estados de loading
  const [savingRepactuacao, setSavingRepactuacao] = useState(false)
  const [deletingRepactuacao, setDeletingRepactuacao] = useState(false)
  ...
  const handleSaveRepactuacao = async () => {
    try {
      ...
      setSavingRepactuacao(false)
    }
  }

  const openDeleteModal = (id: number, indice: string) => {
    setRepactuacaoToDelete({ id, indice })
    setDeleteModalOpen(true)
  }

  const handleDeleteRepactuacao = async () => {
    if (!repactuacaoToDelete) return

    try {
      setDeletingRepactuacao(true)
      await deleteRepactuacao(repactuacaoToDelete.id)
      toast({
        title: "Sucesso",
                    ...
                    <TableCell>
                      <div className="flex space-x-2">
                        ...
                        <Button
                          ...
                          onClick={() => openDeleteModal(repactuacao.idRepactuacao, repactuacao.indice)}
                        >
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          <div className="py-4">
            <p className="text-sm text-muted-foreground">
              Tem certeza que deseja excluir a repactuação <strong>{repactuacaoToDelete?.indice}</strong>?
            </p>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleDeleteRepactuacao} disabled={deletingRepactuacao}>
              {deletingRepactuacao ? (
                <>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveRepactuacao} disabled={savingRepactuacao}>
              {savingRepactuacao ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
