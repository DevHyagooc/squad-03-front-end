...
import type { Documento } from "@/components/DocumentosContrato"
 ...
 */
export const getDocumentosByContrato = async (contratoId: number | string): Promise<Documento[]> => {
  try {
    const response = await fetchWithAuth(`/api/documento/contrato/${contratoId}`, {
      method: "GET",
    })
 ...
 */
export const uploadDocumento = async (contratoId: number | string, file: File): Promise<Documento> => {
  try {
    ...
    const url = `${API_BASE_URL}/api/documento/upload?contratoId=${encodeURIComponent(contratoId.toString())}`
